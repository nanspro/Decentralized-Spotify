...
function LibsWrapper(walletIndex = 0) {
  let libsInstance = null

  const assertLibsDidInit = () => {
    if (!libsInstance) throw new Error('Error: libs not initialized!')
  }
  ...
  this.signUp = async ({ metadata }) => {
    assertLibsDidInit()
    return await libsInstance.Account.signUp(
      metadata.email,
      metadata.password,
      metadata,
      false /* is creator */,
  ...
  this.upgradeToCreator = async ({ endpoint, userNode }) => {
    assertLibsDidInit()
    await libsInstance.User.upgradeToCreator(userNode, endpoint)
  }
  ...
  this.uploadTrack = async ({ trackFile, trackMetadata }) => {
    ...
    const { trackId, error } = await libsInstance.Track.uploadTrack(
      trackFile,
      null /* image */,
      trackMetadata,
      () => {} /* on progress */
  ...
  this.getTrack = async trackId => {
    assertLibsDidInit()
    const tracks = await libsInstance.discoveryProvider.getTracks(
      1 /* Limit */,
      0 /* Ofset */,
      [trackId] /* idsArray */,
      null /* targetUserId */,
